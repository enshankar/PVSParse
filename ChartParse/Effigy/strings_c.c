...
uint32_t lookup_strings_funtype_0(strings_funtype_0_htbl_t htbl, mpz_ptr_t i, uint32_t ihash){
        ...
        uint32_t hashindex = ihash & mask; strings_funtype_0_hashentry_t data = htbl->data[hashindex];
        bool_t keyzero;

        int64_t tmp22101 = mpz_cmp_ui(data.key, 0);
        keyzero = (tmp22101 == 0);
        bool_t keymatch;
        int64_t tmp22102 = mpz_cmp(data.key, i);
        keymatch = (tmp22102 == 0);
...
strings_funtype_0_t dupdate_strings_funtype_0(strings_funtype_0_t a, mpz_ptr_t i, uint32_t v){
        ...
        if (num_entries/3 >  size/5){//resize data
                ...
                for (uint32_t j = 0; j < size; j++){//transfer entries
                        ...
                        bool_t keyzero;
                        int64_t tmp22103 = mpz_cmp_ui(data[j].key, 0);keyzero = (tmp22103 == 0);
                        if (!keyzero || keyhash != 0){
                                uint32_t new_loc = keyhash ^ new_mask;
                                int64_t tmp22104 = mpz_cmp_ui(new_data[new_loc].key, 0);
                                keyzero = (tmp22104 == 0);
                                while (keyzero && new_data[new_loc].keyhash == 0){
                                        new_loc++;
                                        new_loc = new_loc ^ new_mask;

                                        int64_t tmp22105 = mpz_cmp_ui(new_data[new_loc].key, 0);
                                        keyzero = (tmp22105 == 0);
                                }
        ...
        int64_t tmp22106 = mpz_cmp_ui(hentry.key, 0);hentrykeyzero = (tmp22106 == 0);
...
strings_array_2_t upgrade_strings_array_2(strings_array_2_t x, uint32_t i, uint32_t v){
        strings_array_2_t y; 
         if (x->count == 1 && i < x->max){y = x;}
                ...
                y = safe_malloc(sizeof(struct strings_array_2_s) + (newmax * sizeof(uint32_t)));
                y->count = 1;
                y->size = i+1;
                y->max = newmax;
...
uint32_t lookup_strings_funtype_3(strings_funtype_3_htbl_t htbl, uint32_t i, uint32_t ihash){
        uint32_t mask = htbl->size - 1;
        uint32_t hashindex = ihash & mask; strings_funtype_3_hashentry_t data = htbl->data[hashindex];
        bool_t keyzero;
...
strings_funtype_3_t dupdate_strings_funtype_3(strings_funtype_3_t a, uint32_t i, uint32_t v){
        ...
        if (hentrykeyzero && (hkeyhash == 0))
                {htbl->data[hashindex].key = (uint32_t)i; htbl->data[hashindex].keyhash = ihash; htbl->data[hashindex].value = (uint32_t)v;}
            else {uint32_t tempvalue;tempvalue = (uint32_t)htbl->data[hashindex].value;tempvalue = (uint32_t)v;};
        return a;
...
extern uint32_t h_strings_closure_4(uint32_t ivar_6, strings_array_2_t ivar_2){
        uint32_t result;
        uint32_t ivar_8;
        ivar_8 = (uint32_t)ivar_2->elems[ivar_6];
        //copying to uint32 from uint32;
        result = (uint32_t)ivar_8;
...
strings_closure_4_t copy_strings_closure_4(strings_closure_4_t x){
        ...
        if (x->htbl != NULL){
            strings_funtype_3_htbl_t new_htbl = (strings_funtype_3_htbl_t) safe_malloc(sizeof(strings_funtype_3_htbl_t));
            new_htbl->size = x->htbl->size;
            new_htbl->num_entries = x->htbl->num_entries;
            strings_funtype_3_hashentry_t * new_data = (strings_funtype_3_hashentry_t *) safe_malloc(new_htbl->size * sizeof(strings_funtype_3_hashentry_t));
            memcpy(new_data, x->htbl->data, (new_htbl->size * sizeof(strings_funtype_3_hashentry_t)));
...
if (closure->htbl != NULL){
        ...
        bool_t keyzero;
         int64_t tmp22108 = mpz_cmp_ui(entry.key, 0);
         keyzero = (tmp22108 == 0);
        if (!keyzero && entry.keyhash != 0){
            uint32_t result;
            result = (uint32_t)entry.value;
...
strings_closure_5_t copy_strings_closure_5(strings_closure_5_t x){
        ...
        y->fvar_1 = x->fvar_1; x->fvar_1->count++;
        if (x->htbl != NULL){
            strings_funtype_0_htbl_t new_htbl = (strings_funtype_0_htbl_t) safe_malloc(sizeof(strings_funtype_0_htbl_t));
            new_htbl->size = x->htbl->size;
            new_htbl->num_entries = x->htbl->num_entries;
            strings_funtype_0_hashentry_t * new_data = (strings_funtype_0_hashentry_t *) safe_malloc(new_htbl->size * sizeof(strings_funtype_0_hashentry_t));
            memcpy(new_data, x->htbl->data, (new_htbl->size * sizeof(strings_funtype_0_hashentry_t)));
            new_htbl->data = new_data;
            y->htbl = new_htbl;
        } else
            {y->htbl = NULL;};
...
extern strings__string_t strings__make_string(uint32_t ivar_1, strings_array_2_t ivar_2){
        ...
        strings_closure_4_t cl22107;
        cl22107 = new_strings_closure_4();
        cl22107->fvar_1 = (strings_array_2_t)ivar_2;
        cl22107->fvar_1->count++;
        release_strings_array_2(ivar_2);
        ivar_13 = (strings_funtype_3_t)cl22107;
        strings_funtype_0_t ivar_11;
        strings_closure_5_t cl22109;
        cl22109 = new_strings_closure_5();
        cl22109->fvar_1 = (strings_funtype_3_t)ivar_13;
        cl22109->fvar_1->count++;
        ivar_11 = (strings_funtype_0_t)cl22109;
        result = (strings__string_t)new_strings__string();;
        mpz_init(result->length);
        mpz_set(result->length, ivar_3);
        mpz_clear(ivar_3);
        result->seq = (strings_funtype_0_t)ivar_11;
