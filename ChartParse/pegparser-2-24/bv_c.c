...
uint32_t lookup_bv_funtype_0(bv_funtype_0_htbl_t htbl, mpz_ptr_t i, uint32_t ihash){
        ...
        int64_t tmp2532 = mpz_cmp_ui(data.key, 0);
        keyzero = (tmp2532 == 0);
        bool_t keymatch;
        int64_t tmp2533 = mpz_cmp(data.key, i);
        keymatch = (tmp2533 == 0);
...
bv_funtype_0_t dupdate_bv_funtype_0(bv_funtype_0_t a, mpz_ptr_t i, bool_t v, mpz_ptr_t bv__N){
        ...
        if (num_entries/3 >  size/5){//resize data
                ...
                for (uint32_t j = 0; j < size; j++){//transfer entries
                        ...
                        bool_t keyzero;
                        int64_t tmp2534 = mpz_cmp_ui(data[j].key, 0);keyzero = (tmp2534 == 0);
                        if (!keyzero || keyhash != 0){
                                uint32_t new_loc = keyhash ^ new_mask;
                                int64_t tmp2535 = mpz_cmp_ui(new_data[new_loc].key, 0);
                                keyzero = (tmp2535 == 0);
                                while (keyzero && new_data[new_loc].keyhash == 0){
                                        new_loc++;
                                        new_loc = new_loc ^ new_mask;

                                        int64_t tmp2536 = mpz_cmp_ui(new_data[new_loc].key, 0);
                                        keyzero = (tmp2536 == 0);
                                }
        ...
        int64_t tmp2537 = mpz_cmp_ui(hentry.key, 0);hentrykeyzero = (tmp2537 == 0);
...
char* json_bv_funtype_0(bv_funtype_0_t x, mpz_ptr_t bv__N){char * result = safe_malloc(22); sprintf(result, "%s", "\"bv_funtype_0\""); return result;}
...
if (closure->htbl != NULL){
        ...
        bool_t keyzero;
         int64_t tmp2538 = mpz_cmp_ui(entry.key, 0);
         keyzero = (tmp2538 == 0);
        ...
        return h_bv_closure_1(bvar, closure->fvar_1, closure->fvar_2);};
...
bool_t h_bv_closure_1(mpz_ptr_t ivar_3, mpz_ptr_t bv__N, bool_t ivar_1){
        bool_t result;
...
bool_t bv__bvec0(mpz_ptr_t bv__N, mpz_ptr_t ivar_1){
        bool_t  result;
        result = (bool_t) false;
...
bool_t bv__bvec1(mpz_ptr_t bv__N, mpz_ptr_t ivar_1){
        bool_t  result;
        result = (bool_t) true;
...
bv_funtype_0_t bv__fill(mpz_ptr_t bv__N, bool_t ivar_1){
        bv_funtype_0_t  result;
        bv_closure_1_t cl2539;
        cl2539 = new_bv_closure_1();
        mpz_set(cl2539->fvar_1, bv__N);
        cl2539->fvar_2 = (bool_t)ivar_1;
        result = (bv_funtype_0_t)cl2539;

        
        return result;
}

bool_t bv__caret(mpz_ptr_t bv__N, bv_funtype_0_t ivar_1, mpz_ptr_t ivar_2){
        bool_t  result;
