...
void release_dfa_funtype_20(dfa_funtype_20_t x){
        x->count--;
            if (x->count <= 0)

                x->ftbl->rptr(x);}

dfa_funtype_20_t copy_dfa_funtype_20(dfa_funtype_20_t x){return x->ftbl->cptr(x);}

uint32_t lookup_dfa_funtype_20(dfa_funtype_20_htbl_t htbl, uint8_t i, uint32_t ihash){
        uint32_t mask = htbl->size - 1;
        uint32_t hashindex = ihash & mask;
        dfa_funtype_20_hashentry_t data = htbl->data[hashindex];
        bool_t keyzero;
...
dfa_funtype_20_t dupdate_dfa_funtype_20(dfa_funtype_20_t a, uint8_t i, uint8_t v){
        dfa_funtype_20_htbl_t htbl = a->htbl;
        if (htbl == NULL){//construct new htbl
                htbl = (dfa_funtype_20_htbl_t)safe_malloc(sizeof(struct dfa_funtype_20_htbl_s));
                htbl->size = HTBL_DEFAULT_SIZE; htbl->num_entries = 0;
                htbl->data = (dfa_funtype_20_hashentry_t *)safe_malloc(HTBL_DEFAULT_SIZE * sizeof(struct dfa_funtype_20_hashentry_s));
        ...
        uint32_t size = htbl->size;
        uint32_t num_entries = htbl->num_entries;
        dfa_funtype_20_hashentry_t * data = htbl->data;
        if (num_entries/3 >  size/5){//resize data
                uint32_t new_size = 2*size; uint32_t new_mask = new_size - 1;
                if (size >= HTBL_MAX_SIZE) out_of_memory();
                dfa_funtype_20_hashentry_t * new_data = (dfa_funtype_20_hashentry_t *)safe_malloc(new_size * sizeof(struct dfa_funtype_20_hashentry_s));
                for (uint32_t j = 0; j < new_size; j++){
                        new_data[j].key = 0;
                        new_data[j].keyhash = 0;}
                for (uint32_t j = 0; j < size; j++){//transfer entries
                ...
                safe_free(data);}
        uint32_t ihash = uint32_hash(i);
        uint32_t hashindex = lookup_dfa_funtype_20(htbl, i, ihash);
        dfa_funtype_20_hashentry_t hentry = htbl->data[hashindex];
        uint32_t hkeyhash = hentry.keyhash;
        bool_t hentrykeyzero;
        hentrykeyzero = (hentry.key == 0);
...
dfa_funtype_20_t update_dfa_funtype_20(dfa_funtype_20_t a, uint8_t i, uint8_t v){
        if (a->count == 1){
                return dupdate_dfa_funtype_20(a, i, v);
            } else {
                dfa_funtype_20_t x = copy_dfa_funtype_20(a);
                a->count--;
                return dupdate_dfa_funtype_20(x, i, v);
            }}

bool_t equal_dfa_funtype_20(dfa_funtype_20_t x, dfa_funtype_20_t y){
        return false;}

char* json_dfa_funtype_20(dfa_funtype_20_t x){char * result = safe_malloc(24); sprintf(result, "%s", "\"dfa_funtype_20\""); return result;}


uint8_t f_dfa_closure_21(struct dfa_closure_21_s * closure, uint8_t bvar){
if (closure->htbl != NULL){
        dfa_funtype_20_htbl_t htbl = closure->htbl;
        uint32_t hash = uint32_hash(bvar);
        uint32_t hashindex = lookup_dfa_funtype_20(htbl, bvar, hash);
        dfa_funtype_20_hashentry_t entry = htbl->data[hashindex];
        bool_t keyzero;
         keyzero = (entry.key == 0);
...
uint8_t h_dfa_closure_21(uint8_t ivar_4){
        uint8_t result;
        bool_t ivar_5;
...
dfa_closure_21_t new_dfa_closure_21(void){
        static struct dfa_funtype_20_ftbl_s ftbl = {.fptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&f_dfa_closure_21, .mptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&m_dfa_closure_21, .rptr =  (void (*)(dfa_funtype_20_t))&release_dfa_closure_21, .cptr = (dfa_funtype_20_t (*)(dfa_funtype_20_t))&copy_dfa_closure_21};
        dfa_closure_21_t tmp = (dfa_closure_21_t) safe_malloc(sizeof(struct dfa_closure_21_s));
        tmp->count = 1;
        tmp->ftbl = &ftbl;
        tmp->htbl = NULL;
        return tmp;}
...
void release_dfa_closure_21(dfa_funtype_20_t closure){
        dfa_closure_21_t x = (dfa_closure_21_t)closure;
        x->count--;
        if (x->count <= 0){
        //printf("\nFreeing\n");
        safe_free(x);}}
...
dfa_closure_21_t copy_dfa_closure_21(dfa_closure_21_t x){
        dfa_closure_21_t y = new_dfa_closure_21();
        y->ftbl = x->ftbl;

        if (x->htbl != NULL){
            dfa_funtype_20_htbl_t new_htbl = (dfa_funtype_20_htbl_t) safe_malloc(sizeof(struct dfa_funtype_20_htbl_s));
            new_htbl->size = x->htbl->size;
            new_htbl->num_entries = x->htbl->num_entries;
            dfa_funtype_20_hashentry_t * new_data = (dfa_funtype_20_hashentry_t *) safe_malloc(new_htbl->size * sizeof(struct dfa_funtype_20_hashentry_s));
            memcpy(new_data, x->htbl->data, (new_htbl->size * sizeof(struct dfa_funtype_20_hashentry_s)));
            new_htbl->data = new_data;;
            y->htbl = new_htbl;
        } else
            {y->htbl = NULL;};
...
uint8_t h_dfa_closure_22(uint8_t ivar_5){
        uint8_t result;
        bool_t ivar_6;
...
dfa_closure_22_t new_dfa_closure_22(void){
        static struct dfa_funtype_20_ftbl_s ftbl = {.fptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&f_dfa_closure_22, .mptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&m_dfa_closure_22, .rptr =  (void (*)(dfa_funtype_20_t))&release_dfa_closure_22, .cptr = (dfa_funtype_20_t (*)(dfa_funtype_20_t))&copy_dfa_closure_22};
        dfa_closure_22_t tmp = (dfa_closure_22_t) safe_malloc(sizeof(struct dfa_closure_22_s));
        tmp->count = 1;
        tmp->ftbl = &ftbl;
        tmp->htbl = NULL;
        return tmp;}
...
void release_dfa_closure_22(dfa_funtype_20_t closure){
        dfa_closure_22_t x = (dfa_closure_22_t)closure;
        x->count--;
        if (x->count <= 0){
        //printf("\nFreeing\n");
        safe_free(x);}}
...
dfa_closure_22_t copy_dfa_closure_22(dfa_closure_22_t x){
        dfa_closure_22_t y = new_dfa_closure_22();
        y->ftbl = x->ftbl;
...
uint8_t h_dfa_closure_23(uint8_t ivar_4){
        uint8_t result;
        bool_t ivar_5;
...
dfa_closure_23_t new_dfa_closure_23(void){
        static struct dfa_funtype_20_ftbl_s ftbl = {.fptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&f_dfa_closure_23, .mptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&m_dfa_closure_23, .rptr =  (void (*)(dfa_funtype_20_t))&release_dfa_closure_23, .cptr = (dfa_funtype_20_t (*)(dfa_funtype_20_t))&copy_dfa_closure_23};
        dfa_closure_23_t tmp = (dfa_closure_23_t) safe_malloc(sizeof(struct dfa_closure_23_s));
        tmp->count = 1;
        tmp->ftbl = &ftbl;
        tmp->htbl = NULL;
        return tmp;}
...
void release_dfa_closure_23(dfa_funtype_20_t closure){
        dfa_closure_23_t x = (dfa_closure_23_t)closure;
        x->count--;
        if (x->count <= 0){
        //printf("\nFreeing\n");
        safe_free(x);}}
...
dfa_closure_23_t copy_dfa_closure_23(dfa_closure_23_t x){
        dfa_closure_23_t y = new_dfa_closure_23();
        y->ftbl = x->ftbl;
...
uint8_t h_dfa_closure_24(uint8_t ivar_5){
        uint8_t result;
        bool_t ivar_6;
...
dfa_closure_24_t new_dfa_closure_24(void){
        static struct dfa_funtype_20_ftbl_s ftbl = {.fptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&f_dfa_closure_24, .mptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&m_dfa_closure_24, .rptr =  (void (*)(dfa_funtype_20_t))&release_dfa_closure_24, .cptr = (dfa_funtype_20_t (*)(dfa_funtype_20_t))&copy_dfa_closure_24};
        dfa_closure_24_t tmp = (dfa_closure_24_t) safe_malloc(sizeof(struct dfa_closure_24_s));
        tmp->count = 1;
        tmp->ftbl = &ftbl;
        tmp->htbl = NULL;
        return tmp;}
...
void release_dfa_closure_24(dfa_funtype_20_t closure){
        dfa_closure_24_t x = (dfa_closure_24_t)closure;
        x->count--;
        if (x->count <= 0){
        //printf("\nFreeing\n");
        safe_free(x);}}
...
dfa_closure_24_t copy_dfa_closure_24(dfa_closure_24_t x){
        dfa_closure_24_t y = new_dfa_closure_24();
        y->ftbl = x->ftbl;
...
uint8_t h_dfa_closure_25(uint8_t ivar_5){
        uint8_t result;
        result = (uint8_t)1;
...
dfa_closure_25_t new_dfa_closure_25(void){
        static struct dfa_funtype_20_ftbl_s ftbl = {.fptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&f_dfa_closure_25, .mptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&m_dfa_closure_25, .rptr =  (void (*)(dfa_funtype_20_t))&release_dfa_closure_25, .cptr = (dfa_funtype_20_t (*)(dfa_funtype_20_t))&copy_dfa_closure_25};
        dfa_closure_25_t tmp = (dfa_closure_25_t) safe_malloc(sizeof(struct dfa_closure_25_s));
        tmp->count = 1;
        tmp->ftbl = &ftbl;
        tmp->htbl = NULL;
        return tmp;}
...
void release_dfa_closure_25(dfa_funtype_20_t closure){
        dfa_closure_25_t x = (dfa_closure_25_t)closure;
        x->count--;
        if (x->count <= 0){
        //printf("\nFreeing\n");
        safe_free(x);}}
...
dfa_closure_25_t copy_dfa_closure_25(dfa_closure_25_t x){
        dfa_closure_25_t y = new_dfa_closure_25();
        y->ftbl = x->ftbl;
...
uint8_t h_dfa_closure_26(uint8_t ivar_5){
        uint8_t result;
        result = (uint8_t)1;
...
dfa_closure_26_t new_dfa_closure_26(void){
        static struct dfa_funtype_20_ftbl_s ftbl = {.fptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&f_dfa_closure_26, .mptr = (uint8_t (*)(dfa_funtype_20_t, uint8_t))&m_dfa_closure_26, .rptr =  (void (*)(dfa_funtype_20_t))&release_dfa_closure_26, .cptr = (dfa_funtype_20_t (*)(dfa_funtype_20_t))&copy_dfa_closure_26};
        dfa_closure_26_t tmp = (dfa_closure_26_t) safe_malloc(sizeof(struct dfa_closure_26_s));
        tmp->count = 1;
        tmp->ftbl = &ftbl;
        tmp->htbl = NULL;
        return tmp;}
...
void release_dfa_closure_26(dfa_funtype_20_t closure){
        dfa_closure_26_t x = (dfa_closure_26_t)closure;
        x->count--;
        if (x->count <= 0){
        //printf("\nFreeing\n");
        safe_free(x);}}
...
dfa_closure_26_t copy_dfa_closure_26(dfa_closure_26_t x){
        dfa_closure_26_t y = new_dfa_closure_26();
        y->ftbl = x->ftbl;
...
dfa__dfaResult_adt_t dfa__failed(void){
        ...
        if (!defined){
                    uint8_t ivar_1;
        ivar_1 = (uint8_t)0;
        dfa__dfaResult_adt_t tmp4778 = new_dfa__dfaResult_adt();;
        result = (dfa__dfaResult_adt_t)tmp4778;
        tmp4778->dfa__dfaResult_adt_index = (uint8_t)ivar_1;
...
dfa__dfaResult_adt_t dfa__success(uint32_t ivar_2){
        ...
        uint8_t ivar_1;
        ivar_1 = (uint8_t)1;
        dfa__success_t tmp4779 = new_dfa__success();;
        result = (dfa__dfaResult_adt_t)tmp4779;
        tmp4779->dfa__dfaResult_adt_index = (uint8_t)ivar_1;
        tmp4779->span = (uint32_t)ivar_2;
...
dfa_funtype_3_t dfa__reduce_nat(mpz_ptr_t ivar_1, dfa_funtype_4_t ivar_2){
        ...
        dfa_closure_5_t cl4782;
        cl4782 = new_dfa_closure_5();
        cl4782->fvar_1 = (dfa_funtype_4_t)ivar_2;
        if (cl4782->fvar_1 != NULL) cl4782->fvar_1->count++;
        mpz_set(cl4782->fvar_2, ivar_1);
        release_dfa_funtype_4(ivar_2);
        result = (dfa_funtype_3_t)cl4782;
...
dfa_funtype_3_t dfa__REDUCE_nat(dfa_funtype_3_t ivar_1, dfa_funtype_7_t ivar_3){
        dfa_funtype_3_t  result;
        dfa_closure_8_t cl4790;
        cl4790 = new_dfa_closure_8();
        cl4790->fvar_1 = (dfa_funtype_3_t)ivar_1;
        if (cl4790->fvar_1 != NULL) cl4790->fvar_1->count++;
        cl4790->fvar_2 = (dfa_funtype_7_t)ivar_3;
        if (cl4790->fvar_2 != NULL) cl4790->fvar_2->count++;
        release_dfa_funtype_7(ivar_3);
        release_dfa_funtype_3(ivar_1);
        result = (dfa_funtype_3_t)cl4790;
...
dfa_funtype_9_t dfa__reduce_ordinal(ordstruct_adt__ordstruct_adt_t ivar_1, dfa_funtype_10_t ivar_2){
        ...
        dfa_closure_11_t cl4792;
        cl4792 = new_dfa_closure_11();
        cl4792->fvar_1 = (dfa_funtype_10_t)ivar_2;
        if (cl4792->fvar_1 != NULL) cl4792->fvar_1->count++;
        cl4792->fvar_2 = (ordstruct_adt__ordstruct_adt_t)ivar_1;
        if (cl4792->fvar_2 != NULL) cl4792->fvar_2->count++;
        release_dfa_funtype_10(ivar_2);
        release_ordstruct_adt__ordstruct_adt(ivar_1);
        result = (dfa_funtype_9_t)cl4792;
...
dfa_funtype_9_t dfa__REDUCE_ordinal(dfa_funtype_9_t ivar_1, dfa_funtype_12_t ivar_3){
        dfa_funtype_9_t  result;
        dfa_closure_13_t cl4800;
        cl4800 = new_dfa_closure_13();
        cl4800->fvar_1 = (dfa_funtype_9_t)ivar_1;
        if (cl4800->fvar_1 != NULL) cl4800->fvar_1->count++;
        cl4800->fvar_2 = (dfa_funtype_12_t)ivar_3;
        if (cl4800->fvar_2 != NULL) cl4800->fvar_2->count++;
        release_dfa_funtype_12(ivar_3);
        release_dfa_funtype_9(ivar_1);
        result = (dfa_funtype_9_t)cl4800;
...
dfa_funtype_14_t dfa__goodDfaResultp(uint32_t ivar_1, uint32_t ivar_2){
        ...
        dfa_closure_15_t cl4809;
        cl4809 = new_dfa_closure_15();
        cl4809->fvar_1 = (uint32_t)ivar_2;
        cl4809->fvar_2 = (uint32_t)ivar_1;
        result = (dfa_funtype_14_t)cl4809;
...
dfa_array_17_t dfa__digitstate2(void){
        dfa_array_17_t  static  result;

        static bool_t defined = false;
        if (!defined){
                    dfa_funtype_20_t ivar_9;
        dfa_closure_21_t cl4852;
        cl4852 = new_dfa_closure_21();
        ivar_9 = (dfa_funtype_20_t)cl4852;
        //copying to dfa_array_17 from dfa_funtype_20;
        uint32_t tmp4853;
        tmp4853 = (uint32_t)256;
        result = new_dfa_array_17(tmp4853);
        for (uint32_t index_10 = 0; index_10 < tmp4853; index_10++){
             result->elems[index_10] = (uint8_t)ivar_9->ftbl->fptr(ivar_9, index_10);
        };
        release_dfa_funtype_20(ivar_9);
...
dfa_array_17_t dfa__digitstate3(void){
        dfa_array_17_t  static  result;

        static bool_t defined = false;
        if (!defined){
                    dfa_funtype_20_t ivar_10;
        dfa_closure_22_t cl4854;
        cl4854 = new_dfa_closure_22();
        ivar_10 = (dfa_funtype_20_t)cl4854;
        //copying to dfa_array_17 from dfa_funtype_20;
        uint32_t tmp4855;
        tmp4855 = (uint32_t)256;
        result = new_dfa_array_17(tmp4855);
        for (uint32_t index_11 = 0; index_11 < tmp4855; index_11++){
             result->elems[index_11] = (uint8_t)ivar_10->ftbl->fptr(ivar_10, index_11);
        };
        release_dfa_funtype_20(ivar_10);
...
dfa__dfa_t dfa__digitDfa(void){
        ...
        size4865 = (uint32_t)ivar_1;
        size4865 += 0;
        ivar_8 = new_dfa_array_16(size4865);
        uint8_t ivar_4;
        for (uint32_t index4864 = 0; index4864 < size4865; index4864++){
             ivar_4 = (uint8_t)index4864;
             uint8_t ivar_6;
             ivar_6 = (uint8_t)3;
             ivar_8->elems[index4864] = (ivar_4 == ivar_6);
        };
        ...
        uint8_t ivar_19;
        for (uint32_t index4866 = 0; index4866 < 256; index4866++){
             ivar_19 = (uint8_t)index4866;
             bool_t ivar_20;
             ivar_20 = (bool_t)dfa__whitespace((uint8_t)ivar_19);
             ivar_25->elems[index4866] = !ivar_20;
        };
        dfa__dfa_t tmp4867 = new_dfa__dfa();;
        result = (dfa__dfa_t)tmp4867;
        tmp4867->numstates = (uint8_t)ivar_1;
        tmp4867->goodstate = (dfa_array_16_t)ivar_8;
        tmp4867->tbl = (dfa_array_18_t)ivar_13;
        tmp4867->terminal = (dfa_array_16_t)ivar_25;
...
dfa_array_17_t dfa__tokenstate1(void){
        dfa_array_17_t  static  result;

        static bool_t defined = false;
        if (!defined){
                    dfa_funtype_20_t ivar_9;
        dfa_closure_23_t cl4868;
        cl4868 = new_dfa_closure_23();
        ivar_9 = (dfa_funtype_20_t)cl4868;
        //copying to dfa_array_17 from dfa_funtype_20;
        uint32_t tmp4869;
        tmp4869 = (uint32_t)256;
        result = new_dfa_array_17(tmp4869);
        for (uint32_t index_10 = 0; index_10 < tmp4869; index_10++){
             result->elems[index_10] = (uint8_t)ivar_9->ftbl->fptr(ivar_9, index_10);
        };
        release_dfa_funtype_20(ivar_9);
...
dfa_array_17_t dfa__tokenstate2(void){
        dfa_array_17_t  static  result;

        static bool_t defined = false;
        if (!defined){
                    dfa_funtype_20_t ivar_10;
        dfa_closure_24_t cl4870;
        cl4870 = new_dfa_closure_24();
        ivar_10 = (dfa_funtype_20_t)cl4870;
        //copying to dfa_array_17 from dfa_funtype_20;
        uint32_t tmp4871;
        tmp4871 = (uint32_t)256;
        result = new_dfa_array_17(tmp4871);
        for (uint32_t index_11 = 0; index_11 < tmp4871; index_11++){
             result->elems[index_11] = (uint8_t)ivar_10->ftbl->fptr(ivar_10, index_11);
        };
        release_dfa_funtype_20(ivar_10);
...
dfa_array_17_t dfa__tokenstate3(void){
        dfa_array_17_t  static  result;

        static bool_t defined = false;
        if (!defined){
                    dfa_funtype_20_t ivar_6;
        dfa_closure_25_t cl4872;
        cl4872 = new_dfa_closure_25();
        ivar_6 = (dfa_funtype_20_t)cl4872;
        //copying to dfa_array_17 from dfa_funtype_20;
        uint32_t tmp4873;
        tmp4873 = (uint32_t)256;
        result = new_dfa_array_17(tmp4873);
        for (uint32_t index_7 = 0; index_7 < tmp4873; index_7++){
             result->elems[index_7] = (uint8_t)ivar_6->ftbl->fptr(ivar_6, index_7);
        };
        release_dfa_funtype_20(ivar_6);
...
dfa__dfa_t dfa__tokenDfa(void){
        ...
        uint32_t size4883;
        //copying to uint32 from uint8;
        size4883 = (uint32_t)ivar_1;
        size4883 += 0;
        ivar_5 = new_dfa_array_16(size4883);
        uint8_t ivar_4;
        for (uint32_t index4882 = 0; index4882 < size4883; index4882++){
             ivar_4 = (uint8_t)index4882;
             ivar_5->elems[index4882] = (bool_t) false;
        };
        ...
        uint8_t ivar_17;
        for (uint32_t index4884 = 0; index4884 < 256; index4884++){
             ivar_17 = (uint8_t)index4884;
             bool_t ivar_18;
             ivar_18 = (bool_t)dfa__whitespace((uint8_t)ivar_17);
             ivar_23->elems[index4884] = !ivar_18;
        };
        dfa__dfa_t tmp4885 = new_dfa__dfa();;
        result = (dfa__dfa_t)tmp4885;
        tmp4885->numstates = (uint8_t)ivar_1;
        tmp4885->goodstate = (dfa_array_16_t)ivar_5;
        tmp4885->tbl = (dfa_array_18_t)ivar_11;
        tmp4885->terminal = (dfa_array_16_t)ivar_23;
...
dfa_array_17_t dfa__wspacestate1(void){
        dfa_array_17_t  static  result;

        static bool_t defined = false;
        if (!defined){
                    dfa_funtype_20_t ivar_6;
        dfa_closure_26_t cl4886;
        cl4886 = new_dfa_closure_26();
        ivar_6 = (dfa_funtype_20_t)cl4886;
        //copying to dfa_array_17 from dfa_funtype_20;
        uint32_t tmp4887;
        tmp4887 = (uint32_t)256;
        result = new_dfa_array_17(tmp4887);
        for (uint32_t index_7 = 0; index_7 < tmp4887; index_7++){
             result->elems[index_7] = (uint8_t)ivar_6->ftbl->fptr(ivar_6, index_7);
        };
        release_dfa_funtype_20(ivar_6);
...
dfa__dfa_t dfa__wspaceDfa(void){
        ...
        uint32_t size4897;
        //copying to uint32 from uint8;
        size4897 = (uint32_t)ivar_1;
        size4897 += 0;
        ivar_8 = new_dfa_array_16(size4897);
        uint8_t ivar_4;
        for (uint32_t index4896 = 0; index4896 < size4897; index4896++){
             ivar_4 = (uint8_t)index4896;
             uint8_t ivar_6;
             ivar_6 = (uint8_t)2;
             ivar_8->elems[index4896] = (ivar_4 == ivar_6);
        };
        ...
        uint8_t ivar_18;
        for (uint32_t index4898 = 0; index4898 < 256; index4898++){
             ivar_18 = (uint8_t)index4898;
             bool_t ivar_19;
             ivar_19 = (bool_t)dfa__whitespace((uint8_t)ivar_18);
             ivar_24->elems[index4898] = !ivar_19;
        };
        dfa__dfa_t tmp4899 = new_dfa__dfa();;
        result = (dfa__dfa_t)tmp4899;
        tmp4899->numstates = (uint8_t)ivar_1;
        tmp4899->goodstate = (dfa_array_16_t)ivar_8;
        tmp4899->tbl = (dfa_array_18_t)ivar_12;
        tmp4899->terminal = (dfa_array_16_t)ivar_24;
