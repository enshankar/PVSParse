...
uint32_t lookup_gen_strings_funtype_0(gen_strings_funtype_0_htbl_t htbl, mpz_ptr_t i, uint32_t ihash){
        ...
        int64_t tmp2099 = mpz_cmp_ui(data.key, 0);
        keyzero = (tmp2099 == 0);
        bool_t keymatch;
        int64_t tmp2100 = mpz_cmp(data.key, i);
        keymatch = (tmp2100 == 0);
...
gen_strings_funtype_0_t dupdate_gen_strings_funtype_0(gen_strings_funtype_0_t a, mpz_ptr_t i, uint32_t v){
        ...
        if (num_entries/3 >  size/5){//resize data
                ...
                for (uint32_t j = 0; j < size; j++){//transfer entries
                        ...
                        bool_t keyzero;
                        int64_t tmp2101 = mpz_cmp_ui(data[j].key, 0);keyzero = (tmp2101 == 0);
                        if (!keyzero || keyhash != 0){
                                uint32_t new_loc = keyhash ^ new_mask;
                                int64_t tmp2102 = mpz_cmp_ui(new_data[new_loc].key, 0);
                                keyzero = (tmp2102 == 0);
                                while (keyzero && new_data[new_loc].keyhash == 0){
                                        new_loc++;
                                        new_loc = new_loc ^ new_mask;

                                        int64_t tmp2103 = mpz_cmp_ui(new_data[new_loc].key, 0);
                                        keyzero = (tmp2103 == 0);
                                }
        ...
        int64_t tmp2104 = mpz_cmp_ui(hentry.key, 0);hentrykeyzero = (tmp2104 == 0);
...
uint32_t h_gen_strings_closure_2(uint8_t ivar_4){
        uint32_t result;
        uint8_t ivar_5;
        ivar_5 = (uint8_t)0;
...
if (closure->htbl != NULL){
        ...
        bool_t keyzero;
         int64_t tmp2109 = mpz_cmp_ui(entry.key, 0);
         keyzero = (tmp2109 == 0);
        if (!keyzero || entry.keyhash != 0){
            uint32_t result;
            result = (uint32_t)entry.value;
...
uint32_t h_gen_strings_closure_3(mpz_ptr_t ivar_12, mpz_ptr_t ivar_1, gen_strings_funtype_1_t ivar_9){
        uint32_t result;
        uint32_t ivar_10;
        uint8_t ivar_11;
...
uint32_t h_gen_strings_closure_5(uint8_t ivar_5, uint32_t ivar_1){
        uint32_t result;
        //copying to uint32 from uint32;
...
if (closure->htbl != NULL){
        ...
        gen_strings_funtype_0_hashentry_t entry = htbl->data[hashindex];
        bool_t keyzero;
         int64_t tmp2116 = mpz_cmp_ui(entry.key, 0);
         keyzero = (tmp2116 == 0);
        if (!keyzero || entry.keyhash != 0){
            uint32_t result;
            result = (uint32_t)entry.value;
...
uint32_t h_gen_strings_closure_6(mpz_ptr_t ivar_11, mpz_ptr_t ivar_2, gen_strings_funtype_1_t ivar_8){
        uint32_t result;
        uint32_t ivar_9;
        uint8_t ivar_10;
        //copying to uint8 from mpz;
...
if (closure->htbl != NULL){
        ...
        gen_strings_funtype_0_hashentry_t entry = htbl->data[hashindex];
        bool_t keyzero;
         int64_t tmp2162 = mpz_cmp_ui(entry.key, 0);
         keyzero = (tmp2162 == 0);
        if (!keyzero || entry.keyhash != 0){
            uint32_t result;
            result = (uint32_t)entry.value;
...
uint32_t h_gen_strings_closure_7(mpz_ptr_t ivar_14, mpz_ptr_t ivar_2, strings__string_t ivar_1){
        uint32_t result;
        ivar_1->count++;
...
if (closure->htbl != NULL){
        ...
        gen_strings_funtype_0_hashentry_t entry = htbl->data[hashindex];
        bool_t keyzero;
         int64_t tmp2177 = mpz_cmp_ui(entry.key, 0);
         keyzero = (tmp2177 == 0);
        if (!keyzero || entry.keyhash != 0){
            uint32_t result;
            result = (uint32_t)entry.value;
...
uint32_t h_gen_strings_closure_8(mpz_ptr_t ivar_23, mpz_ptr_t ivar_2, strings__string_t ivar_1){
        uint32_t result;
        mpz_ptr_t ivar_35;
...
strings__string_t gen_strings__empty(void){
        ...
        gen_strings_closure_2_t cl2108;
        cl2108 = new_gen_strings_closure_2();
        ivar_9 = (gen_strings_funtype_1_t)cl2108;
        gen_strings_funtype_0_t ivar_7;
        gen_strings_closure_3_t cl2110;
        cl2110 = new_gen_strings_closure_3();
        mpz_set(cl2110->fvar_1, ivar_1);
        cl2110->fvar_2 = (gen_strings_funtype_1_t)ivar_9;
        if (cl2110->fvar_2 != NULL) cl2110->fvar_2->count++;
        ivar_7 = (gen_strings_funtype_0_t)cl2110;
        gen_strings_record_4_t tmp2111 = new_gen_strings_record_4();;
        result = (strings__string_t)tmp2111;
        mpz_init(tmp2111->length);
        mpz_set(tmp2111->length, ivar_1);
        tmp2111->seq = (gen_strings_funtype_0_t)ivar_7;
...
strings__string_t gen_strings__unit(uint32_t ivar_1){
        ...
        gen_strings_closure_5_t cl2115;
        cl2115 = new_gen_strings_closure_5();
        cl2115->fvar_1 = (uint32_t)ivar_1;
        ivar_8 = (gen_strings_funtype_1_t)cl2115;
        gen_strings_funtype_0_t ivar_6;
        gen_strings_closure_6_t cl2117;
        cl2117 = new_gen_strings_closure_6();
        mpz_set(cl2117->fvar_1, ivar_2);
        cl2117->fvar_2 = (gen_strings_funtype_1_t)ivar_8;
        if (cl2117->fvar_2 != NULL) cl2117->fvar_2->count++;
        ivar_6 = (gen_strings_funtype_0_t)cl2117;
        gen_strings_record_4_t tmp2118 = new_gen_strings_record_4();;
        result = (strings__string_t)tmp2118;
        mpz_init(tmp2118->length);
        mpz_set(tmp2118->length, ivar_2);
        tmp2118->seq = (gen_strings_funtype_0_t)ivar_6;
...
mpz_ptr_t gen_strings__strdiff_rec(strings__string_t ivar_1, strings__string_t ivar_2, mpz_ptr_t ivar_3){
        ...
        int64_t tmp2131 = mpz_cmp(ivar_3, ivar_30);
        ivar_27 = (tmp2131 == 0);
        if (ivar_27){ 
             ivar_26 = (bool_t) true;
} else {
             ...
             int64_t tmp2132 = mpz_cmp(ivar_3, ivar_36);
             ivar_33 = (tmp2132 == 0);
             if (ivar_33){  
           ivar_26 = (bool_t) true;
} else {
...
int8_t gen_strings__strcmp(strings__string_t ivar_1, strings__string_t ivar_2){
        ...
        int64_t tmp2147 = mpq_cmp_z(ivar_44, ivar_3);
        ivar_42 = (tmp2147 == 0);
        if (ivar_42){ 
             bool_t ivar_57;
             mpz_ptr_t ivar_58;
             ...
             int64_t tmp2148 = mpz_cmp(ivar_58, ivar_59);
             ivar_57 = (tmp2148 < 0);
             if (ivar_57){  
           ...
           release_strings__string(ivar_2);
           int64_t tmp2149 = mpz_cmp(ivar_64, ivar_65);
           ivar_63 = (tmp2149 > 0);
           if (ivar_63){   
           result = (int8_t)1;
} else {
...
strings__string_t gen_strings__prefix(strings__string_t ivar_1, mpz_ptr_t ivar_2){
        ...
        int64_t tmp2161 = mpz_cmp(ivar_2, ivar_6);
        ivar_4 = (tmp2161 == 0);
        if (ivar_4){ 
             ...
             gen_strings_closure_7_t cl2163;
             cl2163 = new_gen_strings_closure_7();
             mpz_set(cl2163->fvar_1, ivar_2);
             cl2163->fvar_2 = (strings__string_t)ivar_1;
             if (cl2163->fvar_2 != NULL) cl2163->fvar_2->count++;
             release_strings__string(ivar_1);
             ivar_22 = (gen_strings_funtype_0_t)cl2163;
             gen_strings_record_4_t tmp2164 = new_gen_strings_record_4();;
             result = (strings__string_t)tmp2164;
             mpz_init(tmp2164->length);
             mpz_set(tmp2164->length, ivar_2);
             tmp2164->seq = (gen_strings_funtype_0_t)ivar_22;
};
...
strings__string_t gen_strings__suffix(strings__string_t ivar_1, mpz_ptr_t ivar_2){
        ...
        uint8_t ivar_6;
        ivar_6 = (uint8_t)0;
        int64_t tmp2176 = mpz_cmp_ui(ivar_2, ivar_6);
        ivar_4 = (tmp2176 == 0);
        if (ivar_4){ 
             //copying to strings__string from strings__string;
             result = (strings__string_t)ivar_1;
             ...
             gen_strings_closure_8_t cl2178;
             cl2178 = new_gen_strings_closure_8();
             mpz_set(cl2178->fvar_1, ivar_2);
             cl2178->fvar_2 = (strings__string_t)ivar_1;
             if (cl2178->fvar_2 != NULL) cl2178->fvar_2->count++;
             release_strings__string(ivar_1);
             ivar_37 = (gen_strings_funtype_0_t)cl2178;
             gen_strings_record_4_t tmp2179 = new_gen_strings_record_4();;
             result = (strings__string_t)tmp2179;
             mpz_init(tmp2179->length);
             mpz_set(tmp2179->length, ivar_12);
             tmp2179->seq = (gen_strings_funtype_0_t)ivar_37;
};
