...
uint32_t lookup_strings_funtype_0(strings_funtype_0_htbl_t htbl, mpz_ptr_t i, uint32_t ihash){
        ...
        int64_t tmp24042 = mpz_cmp_ui(data.key, 0);
        keyzero = (tmp24042 == 0);
        bool_t keymatch;
        int64_t tmp24043 = mpz_cmp(data.key, i);
        keymatch = (tmp24043 == 0);
...
strings_funtype_0_t dupdate_strings_funtype_0(strings_funtype_0_t a, mpz_ptr_t i, uint32_t v){
        ...
        if (num_entries/3 >  size/5){//resize data
                ...
                for (uint32_t j = 0; j < size; j++){//transfer entries
                        ...
                        bool_t keyzero;
                        int64_t tmp24044 = mpz_cmp_ui(data[j].key, 0);keyzero = (tmp24044 == 0);
                        if (!keyzero || keyhash != 0){
                                uint32_t new_loc = keyhash ^ new_mask;
                                int64_t tmp24045 = mpz_cmp_ui(new_data[new_loc].key, 0);
                                keyzero = (tmp24045 == 0);
                                while (keyzero && new_data[new_loc].keyhash == 0){
                                        new_loc++;
                                        new_loc = new_loc ^ new_mask;

                                        int64_t tmp24046 = mpz_cmp_ui(new_data[new_loc].key, 0);
                                        keyzero = (tmp24046 == 0);
                                }
        ...
        int64_t tmp24047 = mpz_cmp_ui(hentry.key, 0);hentrykeyzero = (tmp24047 == 0);
...
extern uint32_t h_strings_closure_4(uint32_t ivar_7, strings_array_2_t ivar_2, uint32_t ivar_1){
        uint32_t result;
...
if (closure->htbl != NULL){
        ...
        bool_t keyzero;
         int64_t tmp24051 = mpz_cmp_ui(entry.key, 0);
         keyzero = (tmp24051 == 0);
        if (!keyzero || entry.keyhash != 0){
            uint32_t result;
            result = (uint32_t)entry.value;
...
extern uint32_t h_strings_closure_5(mpz_ptr_t ivar_17, mpz_ptr_t ivar_4, uint32_t ivar_1, strings_funtype_3_t ivar_14){
        uint32_t result;
        uint32_t ivar_15;
        uint32_t ivar_16;
...
extern strings__string_t strings__make_string(uint32_t ivar_1, strings_array_2_t ivar_2){
        ...
        strings_closure_4_t cl24050;
        cl24050 = new_strings_closure_4();
        cl24050->fvar_1 = (strings_array_2_t)ivar_2;
        if (cl24050->fvar_1 != NULL) cl24050->fvar_1->count++;
        cl24050->fvar_2 = (uint32_t)ivar_1;
        release_strings_array_2(ivar_2);
        ivar_14 = (strings_funtype_3_t)cl24050;
        strings_funtype_0_t ivar_12;
        strings_closure_5_t cl24052;
        cl24052 = new_strings_closure_5();
        mpz_set(cl24052->fvar_1, ivar_4);
        cl24052->fvar_2 = (uint32_t)ivar_1;
        cl24052->fvar_3 = (strings_funtype_3_t)ivar_14;
        if (cl24052->fvar_3 != NULL) cl24052->fvar_3->count++;
        ivar_12 = (strings_funtype_0_t)cl24052;
        strings__string_t tmp24057 = new_strings__string();;
        result = (strings__string_t)tmp24057;
        mpz_init(tmp24057->length);
        mpz_set(tmp24057->length, ivar_4);
        tmp24057->seq = (strings_funtype_0_t)ivar_12;
